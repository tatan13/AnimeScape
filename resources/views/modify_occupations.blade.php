...
@section('main')
    <div id="addForm">
        <article class="modify_occupations">
            ...
            @endif
            <form action="{{ route('modify_occupations.post', ['anime_id' => $anime->id]) }}" class="modify_occupations_form"
                method="POST">
                @csrf
                    ...
                    <table class="modify_occupations_table">
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select name="main_sub[{{ $loop->iteration }}]">
                                            <option value=""
                                                {{ is_null($occupation->main_sub ?? null) ? 'selected' : '' }}>-
                                            </option>
                                            <option value="{{ \App\Models\Occupation::TYPE_MAIN }}"
                                                {{ $occupation->main_sub == \App\Models\Occupation::TYPE_MAIN ? 'selected' : '' }}>
                                                メイン
                                            </option>
                                            <option value="{{ \App\Models\Occupation::TYPE_SUB }}"
                                                {{ $occupation->main_sub == \App\Models\Occupation::TYPE_SUB ? 'selected' : '' }}>
                                                サブ
                                            </option>
                                            <option value="{{ \App\Models\Occupation::TYPE_OTHERS }}"
                                                {{ $occupation->main_sub == \App\Models\Occupation::TYPE_OTHERS ? 'selected' : '' }}>
                                                その他
                                            </option>
                                        </select>
                                    </td>
                            ...
                            <tr v-for="(text,index) in texts">
                                <td>
                                    <select
                                        :name="'modify_type[' + (index + {{ $anime->occupations->count() }} + 1) + ']'">
                                        <option value="add">追加
                                        </option>
                                        <option value="no_change">変更なし
                                        </option>
            ...
            声優がデータベース未登録の場合、こちらから申請をお願いします→<a
                href="{{ route('add_cast_request.show') }}">声優の追加申請</a>　申請許可まで時間がかかりますがご了承ください。
        </article>
    </div>
@endsection
@section('vue.js')
