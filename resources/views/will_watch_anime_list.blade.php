...
@section('main')
    <article class="will_watch_anime_list">
        <h1>{{ $user->name }}さんの視聴予定表({{ !is_null($year) ? $year . '年' : '' }}{{ (is_null($year) && is_null($coor) ? '全期間' : '') }}{{ !is_null($coor) ? App\Models\Anime::getCoorLabel($coor) . 'クール' : '' }})</h1>
        <div class="title">{{ $user->name }}</div>
        <h2>視聴予定表({{ !is_null($year) ? $year . '年' : '' }}{{ (is_null($year) && is_null($coor) ? '全期間' : '') }}{{ !is_null($coor) ? App\Models\Anime::getCoorLabel($coor) . 'クール' : '' }})</h2>
        <form action="{{ route('user_will_watch_anime_list.show', ['user_id' => $user->id]) }}"
            ...
            <input type="submit" value="絞り込み"> <a
                href="{{ route('user_will_watch_anime_list.show', ['user_id' => $user->id]) }}">絞り込み解除</a>
        </form>
