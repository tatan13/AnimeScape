...
@section('title')
    <title>視聴完了前得点入力済みアニメの視聴完了前データ一括入力 AnimeScape -アニメ批評空間-</title>
    <meta name="robots" content="noindex,nofollow">
@endsection

@section('main')
    <article class="before_score_anime_bulk_before_review">
        <h1>視聴完了前得点入力済みアニメの視聴完了前データ一括入力</h1>
        @if ($errors->any())
        ...
        <section class="search_parameters">
            <h2>検索条件変更</h2>
            <form action="{{ route('before_score_anime_bulk_before_review.show') }}" class="search_parameters_form"
                method="get">
                @csrf
                <select name="year">
            ...
            <a href="{{ route('before_score_anime_bulk_before_review.show') }}">絞り込み解除</a>
        </section>
        <section class="before_score_anime_bulk_before_review_information">
            <h2>注意事項</h2>
            ...
            <h2>{{ !is_null($year) ? $year . '年' : '' }}{{ !is_null($coor) ? App\Models\Anime::getCoorLabel($coor) . 'クール' : '' }}
            </h2>
            <form action="{{ route('before_score_anime_bulk_before_review.show') }}" name="previous" class="d-inline"
                method="get">
                @csrf
            ...
            <form action="{{ route('before_score_anime_bulk_before_review.show') }}" name="next" class="d-inline"
                method="get">
                @csrf
            ...
            @endif
            <form action="{{ route('before_score_anime_bulk_before_review.post') }}"
                class="before_score_anime_bulk_before_review_form" method="POST">
                @csrf
                                    ...
                                    @foreach ($anime_list_coor as $anime)
                                        <tr>
                                            ...
                                            <input type="hidden" name="anime_id[{{ $count }}]" class="anime_id"
                                                value="{{ $anime->id }}">
                                            <td><input type="number" name="before_score[{{ $count }}]"
                                                    class="before_score"
                                            ...
                                            <input type="hidden" name="now_watch[{{ $count }}]" class="now_watch"
                                                value="0">
                                            ...
                                            <input type="hidden" name="give_up[{{ $count }}]" class="give_up"
                                                value="0">
                                            <td><input type="checkbox" name="give_up[{{ $count }}]" class="give_up"
                                                    value="1"
                                                    {{ $anime->userReview->give_up ?? false == true ? 'checked' : '' }}>
                                            </td>
                                            <td><input type="number" name="number_of_watched_episode[{{ $count }}]"
                                            ...
                                            <td><input type="text" name="before_comment[{{ $count }}]"
